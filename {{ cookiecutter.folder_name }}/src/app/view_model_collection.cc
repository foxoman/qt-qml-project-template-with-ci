...
// clang-format on
{
    {{ cookiecutter.cpp_namespace | replace('.', '::') }}::initLibResources();
...
{
    // Sort of a "silly" demo usage of {{ cookiecutter.cpp_namespace | replace('.', '::') }}::Log<>
    Log( str( "rootContext" ), engine )->rootContext()->setContextProperty( "versionInfoBuildDateString", BUILD_ON_DATE );
    // Another sort of "silly" demo usage of {{ cookiecutter.cpp_namespace | replace('.', '::') }}::Log<>
