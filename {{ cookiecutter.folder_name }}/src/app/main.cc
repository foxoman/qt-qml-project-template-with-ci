...
{
    qSetMessagePattern( QString( "[QT-%{type}][v-" ) + {{ cookiecutter.cpp_namespace | replace('.', '::') }}::GIT_HASH_WHEN_BUILT
                        + "][thr:%{threadid}]%{if-category}%{category}: %{endif}%{file}(%{line}): %{message}" );

    // {{ cookiecutter.cpp_namespace | replace('.', '::') }}::log used for DEMONSTRATION. Prefer Qt logging where available.
    {{ cookiecutter.cpp_namespace | replace('.', '::') }}::log( "main.cc starting now %d", static_cast<int>( __LINE__ ) );
